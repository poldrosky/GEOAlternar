...
from <- paste("FROM ", table_name," natural join windinterpolation20_2010_2014 ", sep='')
where <- "WHERE"
...
if(dim(timeserie)[1]!=0){
  ...
  png(filename=paste0(path,'wr20m_',longitude,'_',latitude,'.png'))
  plot(freq)
  dev.off()
  
  wb <- weibull(mast=metmast, v.set=1, print=F)
  png(filename=paste0(path,'wb20m_',longitude,'_',latitude,'.png'))
  plot(wb, show.ak=T)
  dev.off()}
print('done')